...
export function FloatingActions() {
  ...
  useEffect(
    () =>
      onViewportChange(() => {
        ...
        setOverContact(box.top < window.innerHeight && box.bottom > 0);
      }),
    [],
  );
  ...
  return (
    <div
      className={cn(
        "fixed right-4 z-[80] flex flex-col items-end gap-3 transition-[bottom] duration-300 sm:right-6",
        overContact ? "bottom-24 sm:bottom-6" : "bottom-4 sm:bottom-6",
      )}
    >
      <button
        type="button"
        ...
        className={cn(
          "focus-ring-ink grid size-11 place-items-center rounded-full border border-line bg-surface-2 text-ink-1 shadow-md transition-all duration-300 hover:bg-surface",
      ...
      {/* WhatsApp keeps its own brand green — recolouring it would make it
          unrecognisable, and it is the page's primary conversion path. */}
      <a
        href={whatsappUrl}
        target="_blank"
        rel="noreferrer noopener"
        aria-label="Chat with us on WhatsApp"
        className="focus-ring-ink group relative grid h-14 items-center overflow-hidden rounded-full bg-[#25D366] text-white shadow-lg transition-[width] duration-300"
      >
        <span
          aria-hidden="true"
        ...
        <span className="relative flex h-14 items-center gap-3 pl-4 pr-4 md:pr-0 md:transition-[padding] md:duration-300 md:group-hover:pr-5">
          <WhatsAppGlyph />
          <span className="hidden max-w-0 overflow-hidden whitespace-nowrap text-sm font-semibold opacity-0 transition-all duration-300 md:inline md:group-hover:max-w-[10rem] md:group-hover:opacity-100">
            Chat with us
          </span>
        </span>
      </a>
